...
class _CartPageState extends State<CartPage> {
  ...
  _clearCart() async {
    ...
    String prebString2 = "saved_cart";
    // prefs.clear();

    if (_user != "null") prebString2 += _user["user"]["id"].toString();

    String checkCalls = prefs.getString(prebString2) ?? "null";

    if (prefs.containsKey(prebString2)) {
      if (checkCalls != "null") {
        cartList = (json.decode(checkCalls) as List)
            .map((call) => ProductModel.fromJson(call))
            .toList();
      }
    }

    if (favList2.length > 0)
      for (int i = 0; i < favList2.length; i++) {
        cartList.add(favList2[i]);
      }

    String encodeCalls = json.encode(
      cartList.map<Map<String, dynamic>>((call) => call.toMap(call)).toList(),
    );
    // print(encodeCalls);
    prefs.setString(prebString2, encodeCalls);
  }
